    ...
    private void updatePlaying(float delta) {
        ...
        for (int i = 0; i < mEnemy.size(); i++) {
            ...
            for (int j = 0; j < mSteps.size(); j++) {
                Step step = mSteps.get(j);
                if (mEnemy.get(i).getBoundingRectangle().overlaps(step.getBoundingRectangle())) {
                    if(step.mState == step.STEP_STATE_NORMAL) {
                        mEnemy.get(i).setDirection();
                        j = mSteps.size();
                    }
                }
            }
            if(mEnemy.get(i).getY() < 0){
                mEnemy.get(i).setDirection();
            }
